...
#### 為什麼負值能達到 Max-Heap 效果？

`PriorityQueue` 的核心規則是：**priority 越小 → 越先被取出**。

若想讓「值越大的元素越先出列」，只需將原始值取負數作為 priority：
...
`PriorityQueue` 看到的 priority 是 `-20, -10, -5`，因為 `-20 < -10 < -5`，所以出列順序為：

```
-20 → -10 → -5
```

對應原始值即為 `20 → 10 → 5`，正好是**由大到小**的順序。

> **關鍵**：越大的原始數字 → 轉成越小的負數 → 越先被取出，完美模擬 Max-Heap 行為。

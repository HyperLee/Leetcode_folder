...
使用 C# 與 .NET 10 實作 LeetCode 第 19 題，專案目前提供雙指針與計算長度兩種解法，並附上可直接執行的主程式示例，方便對照輸入與輸出。
...
先取得 linked list 的總長度，再把倒數第 `n` 個位置換算成正向索引位置，之後用第二次走訪完成刪除。

**流程**

1. 建立 `dummy -> head`
2. 先走訪一次 linked list，取得總長度 `length`
3. 將倒數第 `n` 個節點換算成前一個節點需要前進的步數 `length - n`
4. 從 `dummy` 出發前進到待刪節點的前一格
5. 將目標節點略過後回傳 `dummy.next`

**為什麼這樣設計**

- 邏輯直觀，先用長度把倒數位置轉成正向位置，容易理解與除錯
- 仍然只需要固定數量指標，空間複雜度維持 `O(1)`
- 配合 `dummy` 節點後，刪除 head 的情況也不需要額外分支

**複雜度**

- 時間複雜度: `O(L)`，會走訪 linked list 兩次
- 空間複雜度: `O(1)`

**與解法一的差異**

- 解法一用雙指針在一次走訪內完成刪除
- 解法二先算長度再定位，步驟較直觀，但需要先完整走訪一次 linked list

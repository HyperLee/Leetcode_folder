...
- [LeetCode 3568 — 清理教室的最少移動](#leetcode-3568--清理教室的最少移動)
  - [目錄](#目錄)
  - [題目說明](#題目說明)
  - [Grid 座標對應](#grid-座標對應)
  - [限制條件](#限制條件)
  - [解題概念與出發點](#解題概念與出發點)
    - [為什麼選擇 BFS？](#為什麼選擇-bfs)
    - [為什麼不能只記錄位置？](#為什麼不能只記錄位置)
    - [為什麼使用 Bitmask？](#為什麼使用-bitmask)
  - [解法：BFS＋Bitmask＋支配剪枝](#解法bfsbitmask支配剪枝)
    - [1. 掃描起點並替垃圾編號](#1-掃描起點並替垃圾編號)
    - [2. 設計 `bestEnergy`](#2-設計-bestenergy)
    - [3. 從 Queue 逐層展開](#3-從-queue-逐層展開)
    - [4. 處理能量與重置區域](#4-處理能量與重置區域)
    - [5. 支配剪枝](#5-支配剪枝)
  - [範例演示流程](#範例演示流程)
  - [正確性說明](#正確性說明)
  - [複雜度分析](#複雜度分析)
...
在二維 grid 中，`x/y` 可能代表幾何座標，也可能只是二維陣列索引的變數名稱，兩種寫法很容易混淆。本題的程式使用較明確的 `row/column`：

| 名稱 | 方向 | 在本題程式中的意義 |
| --- | --- | --- |
| `row` | 垂直方向，由上到下 | 第一個索引，`0 <= row < rowCount` |
| `column` | 水平方向，由左到右 | 第二個索引，`0 <= column < columnCount` |
| `x` | 幾何座標通常代表水平方向 | 對應 `column` |
| `y` | 幾何座標通常代表垂直方向 | 對應 `row` |
...
下面的 ASCII 圖以 3 列、5 欄的 grid 為例。上方的 `x / column` 往右增加，左側的 `y / row` 往下增加：
...
- 陣列索引：`classroom[0][2] == 'L'`
- `row/column` 座標：`(row, column) = (0, 2)`
- 幾何座標：`(x, y) = (2, 0)`

`Program.cs` 中的 `dx`、`dy` 雖然沿用了常見的方向陣列名稱，但實際更新方式是 `dx` 加到 `row`、`dy` 加到 `column`。因此閱讀本題程式時，應以 `row/column` 與 `classroom[row][column]` 為準。README 後文偶爾使用 `(x, y)` 作為位置的抽象寫法；對照實作時，請再依這個對應關係轉換。

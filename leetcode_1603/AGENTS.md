# Repository Guidelines

## 專案結構與命令

本資料夾包含一個巢狀 .NET 10 主控台專案：
`leetcode_1603/leetcode_1603.csproj`。從此題目根目錄執行：

    dotnet build leetcode_1603/leetcode_1603.csproj --nologo
    dotnet run --no-build --project leetcode_1603/leetcode_1603.csproj

先建置後才使用 `--no-build`。VS Code 請使用 `Debug leetcode_1603`；此題沒有
solution、根目錄專案或正式測試專案。

## 程式風格與解法契約

遵守 `.editorconfig`：C# 使用四空格縮排、控制流程保留大括號、明確型別、公開成員
採 PascalCase、區域變數採 camelCase。保留 `Main` 的雙語題目 XML summary，以及
`ParkingSystem` 建構子與 `AddCar` 的繁體中文 XML summary。

唯一公開 API 是 `public sealed class ParkingSystem`、其建構子與
`public bool AddCar(int carType)`。核心物件不輸出；只有 `Main` 可做 console I/O。
三個私有 instance counter 分別保存大、中、小型剩餘車位。成功才扣減相符欄位，拒絕
不得讓計數器低於零，也不得影響其他車種或其他物件。

## 驗證與 Git 範圍

`Main` 是唯一 console I/O 邊界，具有八個確定性 acceptance cases。每案都輸出
`Case`、`Input`、`Expected`、`Actual` 與 `Result: PASS|FAIL`；全部成功的結尾必須是
`Summary: 8/8 checks passed.`，失敗時 exit code 必須為 1。

Git metadata 位於 parent repository 根目錄。任何 commit 或 PR 都必須只涵蓋
`leetcode_1603/`，且不可加入測試專案、套件、並行存取或題目契約外的行為。

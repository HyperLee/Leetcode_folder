...
class Program
{
    ...
    ///   2. 使用 <c>n &amp; 1</c> 判斷當前最低位元是否為 1。
    ///   3. 用變數 <c>last</c> 記錄上一次出現 1 的位元索引（初始為 -1 表示尚未找到）。
    ///   4. 每當在第 i 位找到 1 且 <c>last != -1</c>（已有上一個 1），
    ///      計算距離 <c>i - last</c>，並更新最大距離 <c>res</c>。
    ///   5. 更新 <c>last = i</c>，再將 n 右移一位 <c>n >>= 1</c>，進入下一個位元。

...
### 為什麼相遇後從 `head` 再走一次會找到入口

這一步看起來像是多放一個指標就突然得到答案，但背後其實是距離關係。
假設：
...
`slow` 第一次走到相遇點 `meet` 時，總共走了 `a + b` 步。
`fast` 速度是 `slow` 的兩倍，所以 `fast` 走了 `2(a + b)` 步。
兩者都在 `meet` 時，`fast` 比 `slow` 多走的距離一定是環長的整數倍：
...
```text
head 到入環點的距離 a = 從 meet 回到入環點的距離 c + 若干個完整環長
```

所以相遇後，讓一個新的慢指標從 `head` 出發，原本的慢指標從 `meet` 繼續走，兩者都每次走一步：

- 解法一是 `oneStep2` 從 `head` 出發，`oneStep` 從相遇點繼續走。
- 解法二是 `ptr` 從 `head` 出發，`slow` 從相遇點繼續走。

因為 `head` 走 `a` 步會到入環點，而相遇點走 `c` 步再加上若干圈完整環長也會回到同一個入環點，
所以兩個一步指標第一次再次相遇的位置，就是環的入口。

...
| 特性 | 解法一（優化策略） | 解法二（暴力列舉） |
|------|-------------------|-------------------|
| **核心思路** | 基於均值不等式，找最接近 total/2 的子樹和 | 計算所有可能的分裂乘積，取最大值 |
| **數學原理** | 當兩數之和固定時，越接近相等，乘積越大 | 直接窮舉所有可能性 |
| **實作方式** | 比較 \|cur×2-sum\| 找最優解 | 對每個子樹計算 s×(total-s) |
| **時間複雜度** | O(n) | O(n) |
| **空間複雜度** | O(h) | O(h) |
| **程式碼簡潔度** | 需要判斷邏輯 | 更直觀易懂 |
| **適用場景** | 理解數學優化，面試展示思維 | 直接求解，程式碼簡單 |
...
解法二採用更直觀的方式：遍歷每個可能的分裂點，計算分裂後的乘積，取最大值。

- **第一次 DFS**：計算整棵樹的節點值總和 `total`
- **第二次 DFS**：
  - 計算每個子樹的節點值總和 `s`
  - 移除當前子樹後，剩餘部分的和為 `total - s`
  - 計算乘積 `s × (total - s)` 並更新最大值

#### 程式碼結構

```csharp
private const int MOD = 1_000_000_007;
private long ans = 0;  // 記錄最大乘積

public int MaxProduct_V2(TreeNode root)
{
    ans = 0;  // 重置答案
    int total = DFS1(root);    // 第一次 DFS：計算總和
    DFS2_V2(root, total);      // 第二次 DFS：計算所有可能的乘積
    return (int)(ans % MOD);   // 返回最大乘積取模後的結果
}
```
...
以範例 1 為例，`total = 21`，第二次 DFS 遍歷過程：

| 節點 | 子樹和 s | 剩餘部分 (total-s) | 乘積 s×(total-s) | 是否更新 ans |
|------|---------|-------------------|-----------------|-------------|
| 4 | 4 | 17 | 68 | ans = 68 |
| 5 | 5 | 16 | 80 | ans = 80 |
| 2 | 11 | 10 | **110** | ✅ ans = 110 |
...
- **根節點**：當前節點是根節點時，`s = total`，乘積為 `0`，不影響答案
- **負數點權**：題目保證點權非負，無需特殊判斷。若有負數，需跳過根節點的情況

...
class Program
{
    ...
    ///
    /// 繁體中文
    /// 給定一個 1-indexed 的整數陣列 numbers，該陣列已依非遞減順序排序。請找出兩個數字，使它們的總和等於指定的 target。
    /// 設這兩個數字分別為 numbers[index1] 與 numbers[index2]，其中 1 <= index1 < index2 <= numbers.length。
    /// 請回傳這兩個數字的索引 index1 和 index2，並以長度為 2 的整數陣列 [index1, index2] 表示。
    /// 測試資料保證恰好只有一組解。你不能重複使用同一個元素。
    /// 你的解法必須只使用常數額外空間。
    /// </summary>

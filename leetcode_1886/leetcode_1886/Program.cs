...
class Program
{
    ...
    public bool FindRotation(int[][] mat, int[][] target)
    {
        ...
        for(int k = 0; k < 4; k++)
        {
            ...
            // 外層 i < (n+1)/2：涵蓋「上半部」的行（奇數 n 時天花板除法確保中心行被納入）
            // 內層 j < n/2：涵蓋「左半部」的列（避免與奇數 n 的中心點重複處理）
            for(int i = 0; i < (n + 1) / 2; i++)
            {
                for(int j = 0; j < n / 2; j++)
                {
                    ...
                    // 1. 左上 (i, j) 接收左下 (n-1-j, i) 的值
                    mat[i][j] = mat[n - 1 - j][i];

                    // 2. 左下 (n-1-j, i) 接收右下 (n-1-i, n-1-j) 的值
                    mat[n - 1 - j][i] = mat[n - 1 - i][n - 1 - j];

                    // 3. 右下 (n-1-i, n-1-j) 接收右上 (j, n-1-i) 的值
                    mat[n - 1 - i][n - 1 - j] = mat[j][n - 1 - i];

                    // 4. 右上 (j, n-1-i) 接收最初暫存的左上角的值
                    mat[j][n - 1 - i] = temp;
                }
            }

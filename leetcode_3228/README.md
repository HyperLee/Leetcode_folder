...
- 演算法描述：
	- 解法二採用「偵測 1→0 的轉換」來辨識零段的起點。掃描字串並維護一個計數器 cnt1（表示至目前位置為止已見的 '1' 數量）。當遇到一個字元為 '0' 且其前一字元為 '1' 時，該位置即為一段連續零的開頭，對應地把當前的 cnt1 加到答案中（每一段零只加一次）。
	- 這等價於在遇到每段連續零時計入左側 '1' 的數量，但偵測方式更簡潔：只需檢查相鄰字元的轉換即可。此方法用一個單一掃描即可完成計算。

- 時間與空間複雜度：
	- 時間複雜度：O(n)，其中 n = s.Length。僅對字串做一次線性掃描。
	- 額外空間：實作中使用 ToCharArray() 會造成 O(n) 的額外空間；若改用直接以 s[i] 存取字元，可把額外空間降為 O(1)。

- 逐步範例（範例："010010"）：
	- 初始：cnt1=0, ans=0。
	- i=0: s[0]='0'（無前一字元為 '1'），不處理。
	- i=1: s[1]='1' → cnt1=1。
	- i=2: s[2]='0'，前一字元 s[1]='1'（零段開始）→ ans += cnt1 = 1。
	- i=3: s[3]='0'，前一字元為 '0'（同一段裡面），不再加。
	- i=4: s[4]='1' → cnt1=2。
	- i=5: s[5]='0'，前一字元 s[4]='1'（下一個零段開始）→ ans += cnt1 = 3。
	- 結果：回傳 `3`。

- 改進與注意事項：
	- 輸入驗證：目前實作會在 ToCharArray() 上發生 NullReferenceException 當 s 為 null，建議在方法一開始以 `if (s is null) throw new ArgumentNullException(nameof(s));` 明確處理。若輸入可能包含非 '0'/'1' 的字元，亦建議加入檢查並在非法字元時丟出 `ArgumentException`。
	- 空間優化：若想把額外空間從 O(n) 降為 O(1)，可移除 ToCharArray() 並改為直接使用 s[i]。
	- 溢位風險：若處理極長字串（或題目輸入上界非常大），cnt1 與 ans 的累加可能超出 `int`，此情況下應改用 `long`。

- 與解法一（MaxOperations）的比較：
	- 兩者在邏輯上等價，皆在每個零段累計左側 '1' 的數量；差別在於偵測零段的方式：
		- 解法一：透過跳過整段連續零，在段尾累加一次（明確把整段當成一單位處理）。
		- 解法二：透過判斷前後字元是否由 '1' 轉為 '0' 來辨識零段起點，程式碼更精簡，但需留意 null 與空間使用。

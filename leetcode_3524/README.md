...
當我們刪除前綴 `[1]` 與後綴 `[5]` 時，剩下 `[2,3,4]`。反過來，只要決定要保留哪一段非空連續子陣列，就能唯一決定兩側要刪除的內容：
...
```text
prefix = []       suffix = []          → 保留 [1,2,3,4,5]
prefix = []       suffix = [3,4,5]     → 保留 [1,2]
prefix = [1,2]    suffix = [5]         → 保留 [3,4]
```

唯一不能做的事情是把整個陣列刪光，因為題目要求剩餘陣列必須非空。
...
1. 將其中所有元素相乘。
2. 計算乘積除以 `k` 的餘數。
3. 若餘數是 `x`，就將 `result[x]` 加一。

例如 `nums = [1,2,3,4,5]`、`k = 3` 時，若留下 `[4,5]`：
...
`result` 的長度是 `k`，因為除以 `k` 後的餘數只會落在 `0` 到 `k - 1`。

### 為什麼範例答案是 `[9,2,4]`

對 `nums = [1,2,3,4,5]`、`k = 3`，所有非空連續子陣列的乘積餘數如下：
...
因此，題目原本所說的「刪除 prefix 與 suffix 的操作方式」，可以直接理解成「選擇一個非空連續子陣列」。這也是後面以右端點分組、只追蹤乘積餘數 DP 的出發點。
...
這個方法同時保存「目前右端點的局部狀態」與「所有右端點的累計答案」：

| 陣列 | 代表內容 | 生命週期 |
| --- | --- | --- |
| `dp` | 進入第 `i` 輪時，代表以 `nums[i - 1]` 結尾的子陣列；更新後改為代表以 `nums[i]` 結尾的子陣列 | 跨輪保留 |
| `ndp` | 本輪新建立、以 `nums[i]` 結尾的所有子陣列 | 每輪重新建立 |
| `res` | 已處理的所有右端點中，各乘積餘數的總數量 | 從頭累計到結束 |

因此，`ndp` 必須在每一輪使用 `new long[k]` 重新建立，避免把上一個右端點的狀態直接當成目前右端點的狀態。`dp = ndp` 則是把本輪完成的局部結果交給下一輪使用。

以 `nums = [1,2,3]`、`k = 3` 的 `i = 1` 為例：
...
本輪完成：ndp = [0,0,2]
其中兩個餘數 2 的子陣列是 [2] 與 [1,2]。
```

初始化時 `dp` 全為 `0`，表示尚未產生任何非空子陣列，也因此不會把「空子陣列」計入答案；長度為 1 的子陣列則由 `ndp[nums[i] % k]++` 明確建立。當 `k = 1` 時，所有餘數都只有 `0`，這套轉移也會自然地把每個非空子陣列計入 `res[0]`，不需要額外的特殊分支。

程式中的 `(long)r * nums[i]` 也具有實際意義：雖然 `r` 的範圍只有 `0` 到 `k - 1`，但在 `k = 5`、`nums[i]` 很大時，兩者相乘仍可能超過 C# `int` 的上限，所以要先轉成 `long` 再做乘法。至於計數使用 `long[]`，則是因為所有非空子陣列的總數最多可達 `5,000,050,000`。

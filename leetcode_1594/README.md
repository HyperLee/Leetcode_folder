...
> **行與列的術語說明**
>
> 本專案採用以下慣例（與多數中文數學教材一致）：
>
> | 術語 | 英文 | 方向 | 矩陣中的意義 |
> |---|---|---|---|
> | **列** | Row | 橫向（水平） | `i = 0` 時，固定 row，遍歷所有 column |
> | **行** | Column | 直向（垂直） | `j = 0` 時，固定 column，遍歷所有 row |
>
> 程式碼中迴圈變數 `row` / `col` 代表的是「正在遞增的索引維度」，而**非**正在初始化的那條線：
>
> ```
>               col=0   col=1   col=2   col=3
>             ┌───────┬───────┬───────┬───────┐
>   row=0     │ (0,0) │ (0,1) │ (0,2) │ (0,3) │  ← 第一列 (Row 0)：用 col 遍歷
>             ├───────┼───────┼───────┼───────┤
>   row=1     │ (1,0) │       │       │       │
>             ├───────┼───────┼───────┼───────┤
>   row=2     │ (2,0) │       │       │       │
...
> ```
>
> | 迴圈 | 迴圈變數 | 固定的維度 | 初始化的是 | 到達方式 |
> |---|---|---|---|---|
> | `for(int row ...)` | 遞增 row | col = 0 固定 | 第一**行** (Column 0，直向) | 只能從上方 |
> | `for(int col ...)` | 遞增 col | row = 0 固定 | 第一**列** (Row 0，橫向) | 只能從左方 |

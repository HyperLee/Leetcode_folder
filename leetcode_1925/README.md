...
1) 優化版（雙迴圈 + 平方根檢查）
  - 對於每一對 a 與 b（此實作從 b = a 開始以避免 a, b 的重複計數），計算 c^2 = a^2 + b^2。此做法會以無序方式計算 (a,b)；若要符合題目「有序三元組 (a,b,c)」的定義，我們在最後把無序計數乘以 2（原因是 a == b 的情況在正整數範圍中不會成立）。
...
- `CountTriples(n)`: 優化版本，雙迴圈 + 平方根檢查（回傳有序計數，也就是 (a,b) 與 (b,a) 視為不同）。
...
- 在 n 較小的情況下，兩者在結果上會是一致的（CountTriples 會回傳有序三元組的數目，為 CountTriples2 的結果），

- 但原始的 CountTriples 我們採用迴圈壓縮避免重複並在最後乘 2 以保持有序性。

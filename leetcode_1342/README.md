...
## 補充說明：Lowbit 技巧 `x -= x & -x`

### 原理

`x & -x` 可以取得整數 `x` 中**最低位的 1**（lowest set bit），而 `x -= x & -x` 則是將該位清除。

### -x 如何取得（兩補數）

在電腦中，負數以**兩補數（Two's Complement）**表示：

$$-x = \sim x + 1$$

即：先對 `x` 做位元反轉（NOT），再加 1。
...
```text
 ...
 -x  = 1111 0011
──────────────────
x&-x = 0000 0001  ← 只剩最低位的 1（值為 1）
```

**原因**：從 `x` 變成 `-x` 的過程中，最低位的 1 及其右邊的 0 會被保留，而更高位的位元會全部反轉。因此 AND 後只有最低位的 1 會同時為 1。

### 更多範例

| x | 二進位 | -x 二進位 | x & -x | lowbit 值 |
|---|--------|-----------|--------|-----------|
| 12 | `0000 1100` | `1111 0100` | `0000 0100` | 4 |
| 8 | `0000 1000` | `1111 1000` | `0000 1000` | 8 |
| 7 | `0000 0111` | `1111 1001` | `0000 0001` | 1 |
| 6 | `0000 0110` | `1111 1010` | `0000 0010` | 2 |

### 應用場景

- **計算 popcount**：逐一移除最低位的 1，計算總共有幾個 1
- **遍歷 set bits**：依序取得每個為 1 的位元位置
- **樹狀陣列（BIT/Fenwick Tree）**：利用 lowbit 做區間查詢與更新

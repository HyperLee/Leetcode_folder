...
| 方法二 | 先將陣列整體還原為整數，再與 `k` 相加，最後逐位拆解回陣列 | 邏輯直覺易懂 | 使用 `long` 仍有上限，超大數需改用 `BigInteger` |
...
**出發點**：直覺上最簡單——把陣列變回整數，直接加，再逐位拆解回陣列。

```
[1, 2, 0, 0]  →  1×10³ + 2×10² + 0×10¹ + 0×10⁰  =  1200
1200 + 34 = 1234  →  逐位拆解  →  [1, 2, 3, 4]
```

**步驟：**

1. 遍歷陣列，每個元素乘以對應的 10 的冪次方後累加，還原為 `long` 整數（避免溢位）。
2. `long` 整數與 `k` 直接相加。
3. 將結果逐位拆解（`% 10` 取個位，`/ 10` 右移），每位分開存入 List。
4. 最後反轉 List，得到高位在前的正確格式。

> [!NOTE]
> 使用 `long`（最大約 9.2×10¹⁸）可覆蓋題目限制（`num.length <= 10^4`、`k <= 10^4`）的常見範圍；
> 若需支援任意精度，可改用 `BigInteger`。

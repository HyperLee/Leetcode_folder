...
這是一個以 C# / .NET 10 實作的主控台專案，解決 LeetCode 1550「Three Consecutive Odds」。專案保留兩種解法：逐一檢查固定長度三的視窗，以及維護目前連續奇數區段的滑動視窗。
...
兩個方法採用相同的核心條件，但保存的狀態不同：第一個方法直接重複檢查三個位置；第二個方法記錄最近一個偶數之後的連續奇數長度。
...
兩種方法遇到符合條件的區段都會提早結束，所以最佳情況可能在前幾個元素就完成；最壞情況仍需掃描整個陣列。
...
`Main` 透過 `RunTestCase` 對每個案例呼叫兩個方法，並比較預期與實際結果。案例涵蓋官方範例、剛好三個元素、少於三個元素、全偶數與重複奇數。
...
兩種解法都只讀取輸入陣列，不會修改呼叫者傳入的元素順序或內容。`RunTestCase` 仍會在每次呼叫前建立陣列複本，讓每一種解法都以相同的原始案例獨立執行。

...
`Search3` 不先找旋轉點，而是在每一輪二分搜尋中直接判斷 `mid` 落在哪一段遞增區間。
這裡的「左側」不是指目前搜尋範圍 `l..mid`，而是指旋轉後陣列前半段那段仍然遞增的區間。

以 `nums = [4, 5, 6, 7, 0, 1, 2]` 為例：
...
`nums[0] = 4` 是左側遞增段的第一個值。因為題目保證元素不重複，旋轉後會有這個特性：

- 左側遞增段的值都大於或等於 `nums[0]`，例如 `4, 5, 6, 7`。
- 右側遞增段是原本排序中比較小的值被旋轉到後面，因此都小於 `nums[0]`，例如 `0, 1, 2`。

所以 `if (nums[0] <= nums[mid])` 的用意是判斷 `mid` 是否位於左側遞增段：

- 條件成立，代表 `nums[mid]` 仍然是左側遞增段中的值。
- 條件不成立，也就是 `nums[0] > nums[mid]`，代表 `nums[mid]` 已經落到右側遞增段。

知道 `mid` 位於哪一段之後，就可以利用該段本身有序的特性排除一半範圍：

1. 先檢查 `nums[mid]` 是否等於 `target`，若相等就直接回傳 `mid`。
2. 若 `nums[0] <= nums[mid]`，代表 `mid` 位於左側遞增段。此時 `nums[0]` 到 `nums[mid]` 之間是遞增的，所以可以用 `[nums[0], nums[mid])` 判斷 `target` 是否在 `mid` 左邊。
3. 如果 `nums[0] <= target && target < nums[mid]`，代表答案只可能在 `mid` 左側，將右界縮到 `mid - 1`；否則 `target` 不在這段有序範圍內，改往右側搜尋。
4. 若 `nums[0] > nums[mid]`，代表 `mid` 位於右側遞增段。此時 `nums[mid]` 到 `nums[n - 1]` 之間是遞增的，所以可以用 `(nums[mid], nums[n - 1]]` 判斷 `target` 是否在 `mid` 右邊。
5. 如果 `nums[mid] < target && target <= nums[n - 1]`，代表答案只可能在 `mid` 右側，將左界縮到 `mid + 1`；否則 `target` 不在這段有序範圍內，改往左側搜尋。

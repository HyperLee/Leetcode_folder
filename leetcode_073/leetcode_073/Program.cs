...
class Program
{
    ...
    public void SetZeroes(int[][] matrix)
    {
        ...
        for (int i = 1; i < rows; i++)
        {
            for (int j = 1; j < cols; j++)
            {
                if (matrix[i][j] == 0)
                {
                    matrix[i][0] = 0; // 標記該行需要設為0
                    matrix[0][j] = 0; // 標記該列需要設為0
                }
            }
        }

        // 根據第二行和第二列的標記，將對應的行和列設為0
        for (int i = 1; i < rows; i++)
        {
            for (int j = 1; j < cols; j++)
            {
    ...
    /// <summary>
    /// 解題思路：
    /// 1. 使用兩個布林陣列分別記錄需要設為0的行和列
    /// 2. 第一次遍歷找出所有0的位置，並在對應的布林陣列中標記
    /// 3. 第二次遍歷根據標記將對應的行和列設為0
    /// 
    /// 時間複雜度：O(m*n)，其中 m 和 n 分別是矩陣的行數和列數
    ...
    public void SetZeroes2(int[][] matrix)
    {
        ...
        bool[] row = new bool[m]; // 記錄需要設為0的行
        bool[] col = new bool[n]; // 記錄需要設為0的列
        ...
        for(int i = 0; i < m; i++)
        {
            for(int j = 0; j < n; j++)
            {
                if (matrix[i][j] == 0)
                {
                    row[i] = true; // 標記該行需要設為0
                    col[j] = true; // 標記該列需要設為0
                }
            }
        }
